...
import com.music.bitchord.playback.binaural.BinauralIrs
import java.io.File
...
class PrecisionAudioSinkTest {
    ...
    // ---- End-of-stream drain -------------------------------------------------
    //
    // The binaural spatializer delays its output (~48 ms) and adds a room tail, so
    ...
    private fun binauralChain(): DspChain {
        val root = File(".").canonicalFile
        val asset = listOf(File(root, "app/src/main/assets/binaural/v3_48000.bin"),
            File(root, "src/main/assets/binaural/v3_48000.bin")).first { it.exists() }
        BinauralIrStore.install(asset.inputStream().use { BinauralIrs.read(it) })
        val spatial = SpatialAudioProcessor().apply { enabled = true; mode = SpatialMode.BINAURAL }
    ...
    @Test
    fun `end of stream drains the binaural tail before ending the delegate`() {
        val fakeDelegate = FakeAudioSink()
        val chain = binauralChain()
    ...
    fun `drain resumes across backpressure and holds isEnded off`() {
        val fakeDelegate = FakeAudioSink()
        val sink = createSink(fakeDelegate, dspChain = binauralChain())
        sink.configure(AudioSink.AudioSinkConfig.Builder(rawFormat(C.ENCODING_PCM_FLOAT)).build())
        assertTrue(sink.handleBuffer(floatBurst(2048), 0L, 1))
