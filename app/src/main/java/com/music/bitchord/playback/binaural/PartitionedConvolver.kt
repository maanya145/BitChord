 ...
 * 2 x [block]) are precomputed once per sample rate and shared ([BinauralIrs.Spectra]); each call transforms the new
 * input block of every channel, pushes it into a frequency-domain delay line and sums partition x delayed spectrum
 * products. No latency beyond the block itself: output block k is the convolution result for input block k.
 */
class PartitionedConvolver(private val spectra: BinauralIrs.Spectra) {
