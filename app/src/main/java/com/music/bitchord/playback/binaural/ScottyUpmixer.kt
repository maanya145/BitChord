 ...
 *
 * A port of the reference implementation `scotty.py` (apple-spatial-stereo), itself a line-by-line translation of
 * the STFT upmixer behind macOS "Spatialize Stereo": sqrt-Hann STFT, per-bin covariance of left/right smoothed
 * across frequency and time, ambience = the smaller covariance eigenvalue, steering gains from lookup tables for the
 * front channels and a phantom centre, ambience to the surrounds through all-pass decorrelators and an EQ, and the
 * band below 80 Hz (Linkwitz-Riley) plus the centre into the LFE channel. Float32 throughout, in the same order of
 ...
 */
class ScottyUpmixer(private val t: ScottyTables) {
    val hop: Int = t.hop
    private val n = t.n
    ...
    private val gl = FloatArray(f); private val gr = FloatArray(f)
    private val b480 = FloatArray(f); private val b498 = FloatArray(f)
    private val b4c8 = FloatArray(f); private val b4e0 = FloatArray(f)
    ...
    private val combs = Array(2) { Array(t.combDelays.size) { i -> AllPassComb(ScottyTables.COMB_G, t.combDelays[i]) } }
    ...
    private val lfeScale = (10.0.pow(ScottyTables.LFE_GAIN_DB / 20).toFloat()) * 0.3162277638912201f
    private val surroundGain = 10.0.pow(ScottyTables.SURROUND_GAIN_DB / 20).toFloat()
    private val a2 = ScottyTables.AMBIENCE * ScottyTables.AMBIENCE
    private val b2 = 1f - a2

    init { reset() }

    /**
     * Clears all state, then runs the one hop of silence the reference processes first (it moves the steering-gain
     * smoothers off zero, so skipping it would change the first few hundred milliseconds).
     */
    fun reset() {
    ...
    /** Frequency smoothing: out[k] = sum_j kernel_k[j] x[start_k + j] (double accumulation, like the reference). */
    ...
    private fun spectral() {
        ...
        val x = ScottyTables.CROSSTALK
        val wCoef = x * x + x * -2f
        val lk = ScottyTables.LEAK
        for (k in 0 until f) {
            ...
            gl[k] = glk; gr[k] = grk
            cim *= x; cre *= -x
            b480[k] = grk * cre; b498[k] = grk * cim
            cim = -cim
            b4c8[k] = glk * cre; b4e0[k] = glk * cim
            val dl = max(ll - amb, 0f); val dr = max(rr - amb, 0f)
            val v600 = (dl * (1f - lk) + EPS) + ll * lk
            val v618 = (dr * (1f - lk) + EPS) + rr * lk
            val ratio = v618 / v600
        ...
        val s = ScottyTables.AMBIENCE
        val cg = ScottyTables.CENTRE_GAIN
        for (k in 0 until f) {
            ...
            var zr = b480[k] * rrk - b498[k] * ri
            var zi = b480[k] * ri + b498[k] * rrk
            sre[4][k] = (zr + gl[k] * lr) * s; sim[4][k] = (zi + gl[k] * li) * s
            zr = b4c8[k] * lr - b4e0[k] * li
            zi = b4c8[k] * li + b4e0[k] * lr
...
/**
 * Schroeder all-pass comb as the reference applies it: `y[n] = g x[n] + x[n-D] - g y[n-D]` (g = +0.3), double
