...
/**
 * Every runtime table of the stereo -> 5.1 upmixer, generated from its parameters for one sample rate and frame
 * size. A line-by-line port of `scotty_gen.py` (apple-spatial-stereo), which reproduces the reference upmixer's
 * tables; Float32 arithmetic is kept where the reference used it so the results match to the last bits.
 *
 * @param sampleRate stream rate in Hz
 * @param n STFT frame size (hop = n / 2); 2048 up to 48 kHz, scaled with the rate above that
 */
class ScottyTables(val sampleRate: Int, val n: Int = frameSizeFor(sampleRate)) {
    val hop: Int = n / 2
    ...
    init {
        // ---- frequency-smoothing kernels (FUN_00437d98) ------------------------------------------------------
        val f = bins
        ...
        // ---- covariance / steering-gain time constants (FUN_00437d98, FUN_00438c08) --------------------------
        ...
        for (pan in 0 until 81) {
            ...
            for (ph in 0 until 91) {
                ...
                t660[ph * 81 + pan] = ramp * (-1.0 / (PHASE_WIDTH + PHASE_WIDTH)).toFloat() + (-1.0 / (WIDTH + WIDTH)).toFloat() * ap
            }
        }
        for (i in 0 until LUT_SIZE) {
            val e = exp(t660[i].toDouble()).toFloat()
            lutCentre[i] = e * ((hiG - mn) * P208) + mn * P208
    ...
    companion object {
        ...
        // aufx-scty-appl-headphone-general-stereo, as applied (see APPLE_SPATIAL_STEREO.md)
