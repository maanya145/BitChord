package com.music.bitchord.playback.binaural

import android.content.Context
import android.util.Log
import java.io.DataInputStream
import java.io.InputStream
import java.nio.ByteBuffer
import java.nio.ByteOrder
import kotlin.math.PI
import kotlin.math.abs
import kotlin.math.ceil
import kotlin.math.floor
import kotlin.math.min
import kotlin.math.sin
import kotlin.math.sqrt

/**
 * The binaural spatializer's 6 x 2 impulse responses (L R C LFE Ls Rs -> left / right ear) at one sample rate.
 *
 * Shipped as `assets/binaural/v3_<rate>.bin` for 44.1 and 48 kHz: the "v3" open spatializer built from Meta's SS2
 * HATS head-related impulse responses (Meta Reality Labs Research, CC BY 4.0) and a synthetic listening room,
 * rendered through a reimplementation of the macOS "Spatialize Stereo" spatializer (see `docs/binaural-audio.md`).
 * Other rates are resampled from the nearest family on first use.
 */
class BinauralIrs(val sampleRate: Int, val taps: Int, private val data: FloatArray) {
    init { require(data.size == INPUTS * EARS * taps) }

    fun response(input: Int, ear: Int): FloatArray =
        data.copyOfRange((input * EARS + ear) * taps, (input * EARS + ear + 1) * taps)

    /** Partition spectra for a convolution block size, shared by every renderer at this rate. */
    class Spectra(val block: Int, val partitions: Int) {
        val re = Array(BinauralIrs.INPUTS) { Array(BinauralIrs.EARS) { Array(partitions) { FloatArray(block + 1) } } }
        val im = Array(BinauralIrs.INPUTS) { Array(BinauralIrs.EARS) { Array(partitions) { FloatArray(block + 1) } } }
        /** Leading partitions that carry energy, per (input, ear); the rest are skipped. */
        val activePartitions = Array(BinauralIrs.INPUTS) { IntArray(BinauralIrs.EARS) }
    }

    private val spectraCache = HashMap<Int, Spectra>()

    @Synchronized
    fun spectra(block: Int): Spectra = spectraCache.getOrPut(block) {
        val parts = (taps + block - 1) / block
        val s = Spectra(block, parts)
        val fft = RealFft(2 * block)
        val frame = FloatArray(2 * block)
        for (c in 0 until INPUTS) for (e in 0 until EARS) {
            val base = (c * EARS + e) * taps
            var last = 0
            for (p in 0 until parts) {
                frame.fill(0f)
                val n = min(block, taps - p * block)
                var nonzero = false
                for (i in 0 until n) {
                    val v = data[base + p * block + i]
                    frame[i] = v
                    if (v != 0f) nonzero = true
                }
                if (nonzero) last = p + 1
                fft.forward(frame, s.re[c][e][p], s.im[c][e][p])
            }
            s.activePartitions[c][e] = last
        }
        s
    }

    /** Kaiser-windowed sinc resampling of every response to [target] Hz (band-limited when going down). */
    fun resampledTo(target: Int): BinauralIrs {
        if (target == sampleRate) return this
        val ratio = target.toDouble() / sampleRate
        val outTaps = ceil(taps * ratio).toInt()
        val cutoff = min(1.0, ratio)
        val half = ceil(HALF_WIDTH / cutoff).toInt()
        val out = FloatArray(INPUTS * EARS * outTaps)
        val i0Beta = besselI0(KAISER_BETA)
        for (ch in 0 until INPUTS * EARS) {
            val src = ch * taps
            val dst = ch * outTaps
            for (n in 0 until outTaps) {
                val center = n / ratio
                val k0 = floor(center).toInt() - half
                var acc = 0.0
                for (k in k0..k0 + 2 * half + 1) {
                    if (k < 0 || k >= taps) continue
                    val x = center - k
                    val u = x / (half + 1)
                    if (abs(u) >= 1.0) continue
                    val arg = PI * cutoff * x
                    val sinc = if (abs(arg) < 1e-12) 1.0 else sin(arg) / arg
                    val win = besselI0(KAISER_BETA * sqrt(1 - u * u)) / i0Beta
                    acc += data[src + k] * cutoff * sinc * win
                }
                out[dst + n] = acc.toFloat()
            }
        }
        return BinauralIrs(target, outTaps, out)
    }

    companion object {
        const val INPUTS = 6
        const val EARS = 2
        private const val MAGIC = 0x33424342 // "BCB3" little-endian
        private const val HALF_WIDTH = 32.0
        private const val KAISER_BETA = 8.6

        /** Parses the asset format written by `export_assets.py`. */
        fun read(input: InputStream): BinauralIrs {
            val bytes = DataInputStream(input).use { it.readBytes() }
            val bb = ByteBuffer.wrap(bytes).order(ByteOrder.LITTLE_ENDIAN)
            require(bb.int == MAGIC) { "not a binaural IR file" }
            val version = bb.int
            require(version == 1) { "unsupported binaural IR version $version" }
            val rate = bb.int
            val inputs = bb.int
            val ears = bb.int
            val taps = bb.int
            require(inputs == INPUTS && ears == EARS) { "expected 6 x 2 responses, got $inputs x $ears" }
            val data = FloatArray(inputs * ears * taps)
            bb.asFloatBuffer().get(data)
            return BinauralIrs(rate, taps, data)
        }

        private fun besselI0(x: Double): Double {
            var sum = 1.0
            var term = 1.0
            val q = x * x / 4
            var k = 1
            while (term > 1e-12 * sum) {
                term *= q / (k.toDouble() * k)
                sum += term
                k++
            }
            return sum
        }
    }
}

/**
 * Process-wide cache of [BinauralIrs] per sample rate, loaded from the app's assets. [init] must run before the
 * first binaural render (PlaybackService does it in onCreate); until then — and for rates outside
 * [MIN_RATE]..[MAX_RATE] — [forRate] returns null and the spatial processor falls back to stereo widening.
 */
object BinauralIrStore {
    private const val TAG = "BinauralIrStore"
    const val MIN_RATE = 22050
    const val MAX_RATE = 192000

    @Volatile
    private var context: Context? = null
    private val byRate = HashMap<Int, BinauralIrs>()
    private val shipped = HashMap<Int, BinauralIrs>()

    fun init(context: Context) {
        this.context = context.applicationContext
    }

    /** For tests: supply the shipped responses directly instead of reading them from assets. */
    @Synchronized
    fun install(irs: BinauralIrs) {
        shipped[irs.sampleRate] = irs
        byRate.clear()
    }

    @Synchronized
    fun forRate(sampleRate: Int): BinauralIrs? {
        if (sampleRate !in MIN_RATE..MAX_RATE) return null
        byRate[sampleRate]?.let { return it }
        // 44.1 kHz family for multiples of 11025, 48 kHz family otherwise
        val family = if (sampleRate % 11025 == 0) 44100 else 48000
        val base = shippedFor(family) ?: return null
        val irs = try {
            base.resampledTo(sampleRate)
        } catch (t: Throwable) {
            Log.w(TAG, "resampling binaural responses to $sampleRate Hz failed", t)
            return null
        }
        byRate[sampleRate] = irs
        return irs
    }

    private fun shippedFor(rate: Int): BinauralIrs? {
        shipped[rate]?.let { return it }
        val ctx = context ?: return null
        return try {
            ctx.assets.open("binaural/v3_$rate.bin").use { BinauralIrs.read(it) }.also { shipped[rate] = it }
        } catch (t: Throwable) {
            Log.w(TAG, "binaural responses for $rate Hz unavailable", t)
            null
        }
    }
}
